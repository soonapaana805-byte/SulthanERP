...
public class PaymentService : IPaymentService
{
        ...
        UpdatePaymentDto dto)
    {
        ...
        payment.BalanceAmount =
            payment.GrandTotal - dto.PaidAmount;
    ...
    public async Task<PaymentSummaryDto> GetSummaryAsync()
    {
        ...
        var paymentLines = payments
            .SelectMany(payment =>
                payment.Allocations.Count > 0
                    ? payment.Allocations.Select(x =>
                        (x.PaymentMethod, x.Amount))
                    : new[]
                    {
                        (payment.PaymentMethod, payment.PaidAmount)
                    })
            .ToList();
        ...
        return new PaymentSummaryDto
        {
            ...
            TotalCash = paymentLines
                .Where(x =>
                    x.PaymentMethod == PaymentMode.Cash)
                .Sum(x => x.Amount),

            TotalCard = paymentLines
                .Where(x =>
                    x.PaymentMethod == PaymentMode.Card)
                .Sum(x => x.Amount),

            TotalUpi = paymentLines
                .Where(x =>
                    x.PaymentMethod == PaymentMode.Upi)
                .Sum(x => x.Amount),

            TotalMixed = payments
                .Where(x =>
                    x.PaymentMethod == PaymentMode.Mixed)
                .Sum(x => x.PaidAmount),
